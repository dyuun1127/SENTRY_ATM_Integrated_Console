...
class TestEmergency:
    ...
    def test_declaration_preserves_the_compressed_motion_path(self, plan):
        """선언 앵커는 상태만 바꾸며 원래의 XYZ 운동을 끊지 않는다."""
        declared = plan.definition.events[0].scheduled_time_utc
        fighter = next(a for a in plan.definition.aircraft if a.aircraft_id == FIGHTER)
        declaration_anchors = [
            a for a in fighter.scheduled_states if a.timestamp_utc == declared
        ]
        assert len(declaration_anchors) == 1

        # 선언 앵커를 제외하면 원래의 압축된 운동 경로가 남는다. 두 런타임으로
        # 선언 직전/정각/직후와 다음 운동 앵커 양쪽을 비교한다.
        motion_anchors = tuple(
            a for a in fighter.scheduled_states if a.timestamp_utc != declared
        )
        next_motion = next(a.timestamp_utc for a in motion_anchors if a.timestamp_utc > declared)
        timestamps = {
            declared - timedelta(microseconds=1),
            declared,
            declared + timedelta(microseconds=1),
            declared + (next_motion - declared) / 2,
            next_motion - timedelta(microseconds=1),
            next_motion,
            next_motion + timedelta(microseconds=1),
            *(a.timestamp_utc for a in motion_anchors),
        }
        for timestamp in sorted(timestamps):
            clock = SimulationClock(start_time_utc=timestamp)
            current = SyntheticAircraftRuntime(
                clock=clock,
                initial_state=fighter.initial_state,
                scheduled_states=fighter.scheduled_states,
                presence=fighter.presence,
            ).current_state
            original = SyntheticAircraftRuntime(
                clock=clock,
                initial_state=fighter.initial_state,
                scheduled_states=motion_anchors,
                presence=fighter.presence,
            ).current_state
            assert current is not None and original is not None
            assert current.emergency_status is (
                EmergencyStatus.DECLARED if timestamp >= declared else EmergencyStatus.NONE
            )
            assert current.x_nm == pytest.approx(original.x_nm, rel=0, abs=1e-10)
            assert current.y_nm == pytest.approx(original.y_nm, rel=0, abs=1e-10)
            assert current.altitude_ft == pytest.approx(original.altitude_ft, rel=0, abs=1e-10)
            assert replace(
                current,
                x_nm=original.x_nm,
                y_nm=original.y_nm,
                altitude_ft=original.altitude_ft,
                emergency_status=original.emergency_status,
                emergency_type=original.emergency_type,
            ) == original
