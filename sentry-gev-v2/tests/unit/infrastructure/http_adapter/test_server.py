...
@pytest.mark.parametrize("control, expected_status", [("local", 403), ("any", 200)])
def test_loopback_tunnel_obeys_explicit_control_mode(control, expected_status) -> None:
    """A real loopback socket must not bypass forwarding-aware access checks."""
    server = create_local_golden_demo_server(
        LocalGoldenDemoServerSettings(port=0, scenario="golden", control=control)
    )
    worker = Thread(target=server.serve_forever, daemon=True)
    worker.start()
    ...
    try:
        connection.request("GET", "/api/v1/reference/access", headers=headers)
        response = connection.getresponse()
        assert json.loads(response.read())["operator"] is (control == "any")
        connection.request(
            "POST", "/api/v1/golden-demo/session/commands",
            body=json.dumps({"command": "START"}), headers=headers,
        )
        response = connection.getresponse()
        response.read()
        ...
        assert (session["stage"] == "READY") is (control == "local")
    finally:
        connection.close()
        server.shutdown()
