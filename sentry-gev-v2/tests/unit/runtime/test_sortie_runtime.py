...
from sentry_atm.domain import EmergencyStatus, EmergencyType, OperationalPriorityLevel
...
class TestEmergencyInFlight:
    ...
    def test_declaration_status_and_priority_change_at_the_same_instant(self, plan):
        """단계 7 정각에 우선순위만 비상이고 항공기 상태는 정상인 틈이 없어야 한다."""
        runtime = build_sortie_runtime()
        runtime.simulation.clock.play()
        ...
        for advance, expected_status, expected_priority in (
            (declaration_offset - 1, EmergencyStatus.NONE, OperationalPriorityLevel.ROUTINE),
            (1, EmergencyStatus.DECLARED, OperationalPriorityLevel.EMERGENCY),
            (1, EmergencyStatus.DECLARED, OperationalPriorityLevel.EMERGENCY),
        ):
            result = orchestrator.step(advance_steps=advance)
            fighter = next(s for s in result.traffic_snapshot.states if s.aircraft_id == FIGHTER)
            priority = next(a for a in result.priority_assessments if a.aircraft_id == FIGHTER)
            assert fighter.emergency_status is expected_status
            assert fighter.emergency_type is (
                EmergencyType.PRIORITY_RETURN if expected_status is EmergencyStatus.DECLARED
                else None
            )
            assert priority.priority_level is expected_priority
