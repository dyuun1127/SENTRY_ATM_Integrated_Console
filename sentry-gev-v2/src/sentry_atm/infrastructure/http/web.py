...
_PREDICTION_PATH = "/api/v1/prediction"
_SECURITY_HEADERS = (
...
class GoldenDemoWebWsgiApp:
    ...
    def _access_asset(self, environ: dict[str, object]) -> _StaticAsset:
        ...
        payload = {
            "external": external,
            "control": control,
            # Tunnel requests retain their forwarding headers on loopback.
            "operator": is_operator(environ, control=control),
        }
        return _StaticAsset(
            json.dumps(payload, ensure_ascii=False).encode("utf-8"),
            "application/json; charset=utf-8",
    ...
    def __call__(
        ...
        elif path == _PREDICTION_PATH and self._session_runtime is not None:
            from sentry_atm.infrastructure.http.prediction import current_prediction_payload

            asset = _StaticAsset(
                json.dumps(current_prediction_payload(self._session_runtime),
