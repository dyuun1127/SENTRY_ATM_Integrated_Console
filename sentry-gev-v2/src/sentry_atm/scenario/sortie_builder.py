...
def _with_declaration_anchor(
    states: list[AircraftState], declared: datetime
) -> list[AircraftState]:
    """운동 앵커 사이의 비상 선언을 정확한 시각에 반영한다."""
    index = bisect_left(states, declared, key=lambda state: state.timestamp_utc)
    if index == 0 or (index < len(states) and states[index].timestamp_utc == declared):
        return states

    # SyntheticAircraftRuntime과 같은 등속 전파로 상태만 전환한다. 원본 표본의
    # 위치를 새로 가져오면 관측잡음까지 다시 적용해 선언 순간 항적이 튄다.
    previous = states[index - 1]
    elapsed = (declared - previous.timestamp_utc).total_seconds()
    distance = knots_to_nm_per_second(previous.ground_speed_kt) * elapsed
    heading = radians(previous.heading_deg)
    declaration = replace(
        previous,
        timestamp_utc=declared,
        x_nm=previous.x_nm + distance * sin(heading),
        y_nm=previous.y_nm + distance * cos(heading),
        altitude_ft=(
            previous.altitude_ft + fpm_to_ft_per_second(previous.vertical_speed_fpm) * elapsed
        ),
        emergency_status=EmergencyStatus.DECLARED,
        emergency_type=EmergencyType.PRIORITY_RETURN,
    )
    return [*states[:index], declaration, *states[index:]]
...
def build_sortie_plan(
    *,
    ...
    for callsign, legs in by_callsign.items():
        ...
        if emergency_from_t_s is not None:
            states = _with_declaration_anchor(
                states, SORTIE_START_UTC + timedelta(seconds=emergency_from_t_s)
            )
        aircraft.append(
            ScenarioAircraft(
