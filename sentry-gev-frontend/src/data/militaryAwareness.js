...
// Additional aircraft remain source-owned and never enter the observed-flight total.
const aircraftSources = new Map();
function sourceRegistered(id) {
  return !state.dataManager?.layers || state.dataManager.layers.has(id);
}
function aircraftSource(id) {
  if (id === 'flights') return flightsLayer;
  if (id === 'military') return militaryFlightsLayer;
  return aircraftSources.get(id)?.layer || null;
}
function aircraftSourceIds() {
  return [...AIRCRAFT_DEPENDENCIES, ...aircraftSources.keys()].filter(sourceRegistered);
}
function dependencyIds() {
  return [...DEPENDENCIES, ...aircraftSources.keys()].filter(sourceRegistered);
}
...
function hidePanel() {
  if (state.panel) {
    const includesMapped = DEFERRED_DEPENDENCIES.some(sourceRegistered);
    const prompt = includesMapped ? 'SELECT A FLIGHT, VESSEL, OR MAPPED INSTALLATION' : 'SELECT AN AIRCRAFT';
    const scope = includesMapped ? 'ENABLE TO LOAD OBSERVED / MAPPED PROXIMITY' : 'ENABLE TO LOAD AIRCRAFT PROXIMITY';
    const markup = `<div class="military-awareness-standby">
      <strong>${state.enabled ? 'CONTEXT READY' : 'GLOBAL CONTEXT OFF'}</strong>
      <span>${state.enabled ? prompt : scope}</span>
    </div>`;
...
function collectSourceStates() {
  return Object.fromEntries(dependencyIds().map((layerId) => [layerId, sourceState(layerId)]));
}

function sourceRevision(sourceStates) {
  return dependencyIds().map((layerId) => {
...
export function buildAwarenessContextSnapshot(results, navigation = {}, { subjectPresent = true } = {}) {
  ...
  return {
    ...
    cohorts: results.cohorts.map((cohort) => ({
      ...
      ...(cohort.simulation ? { simulation: true } : {}),
...
export function collectAircraftProximityWindow(position, {
  ...
  const flights = sourceRegistered('flights') ? flightsLayer
  ...
  const military = sourceRegistered('military') ? militaryFlightsLayer
    .getNearby(position, radiusM, AWARENESS_QUERY_LIMIT, { includeHidden: true })
...
function evaluateSubject(subject, sourceStates = collectSourceStates()) {
  ...
  if (!position) return null;
  // Keep the observed feeds and simulation sources in separate cohorts.
  const { flights, military } = collectAircraftProximityWindow(position, { subject });
  const cohorts = [];
  for (const [id, label, items] of [
    ['flights', 'Flights', flights], ['military', 'Military flights', military],
  ]) {
    if (!sourceRegistered(id)) continue;
    const current = sourceStates[id];
    cohorts.push({ id, label, source: current.stats.source || SOURCE_LABEL[id],
      summary: summarizeAwarenessCohortForNavigation(items, current) });
  }
  for (const [id, { layer, label, source }] of aircraftSources) {
    if (!sourceRegistered(id)) continue;
    const items = layer.getNearby(position, AWARENESS_RADIUS_M, AWARENESS_QUERY_LIMIT,
      { includeHidden: true }).filter((item) => !isSame(subject, item, id, 'id'));
    const summary = summarizeAwarenessCohortForNavigation(items, sourceStates[id]);
    if (summary.count !== null) summary.reason = items.length
      ? 'simulation snapshot proximity; separate from live observations'
      : 'no other simulation aircraft in this snapshot window';
    cohorts.push({ id, label, source, simulation: true, summary });
  }
  if (sourceRegistered('ais-live-vessels')) {
    const current = sourceStates['ais-live-vessels'];
    const items = aisLiveVesselsLayer.getNearby(position, AWARENESS_RADIUS_M, AWARENESS_QUERY_LIMIT)
      .filter((item) => !isSame(subject, item, 'ais-live-vessels', 'mmsi'));
    cohorts.push({ id: 'ais-live-vessels', label: 'AIS vessels',
      source: current.stats.source || SOURCE_LABEL['ais-live-vessels'],
      summary: summarizeAwarenessCohortForNavigation(items, current) });
  }
  if (sourceRegistered('military-installations')) {
    const current = sourceStates['military-installations'];
    const items = militaryInstallationsLayer.getNearby(position, AWARENESS_RADIUS_M, AWARENESS_QUERY_LIMIT)
      .filter((item) => !isSame(subject, item, 'military-installations', 'id'));
    cohorts.push({ id: 'military-installations', label: 'Mapped installations',
      source: current.stats.source || SOURCE_LABEL['military-installations'],
      coverage: 'CURRENT VIEWPORT ONLY', summary: summarizeInstallationViewport(items, current) });
  }
  return { subject, evaluatedAt: Date.now(), radiusM: AWARENESS_RADIUS_M, cohorts };
}
...
function focusNearbyTarget(layerId, id, { origin = 'programmatic' } = {}) {
  if (!layerId || !id || !sourceRegistered(layerId)) return false;
  const additional = aircraftSources.get(layerId)?.layer;
  if (additional) return additional.refocusTrackedById?.(id, { origin })
    || additional.trackById(id, { origin });
  if (layerId === 'flights') {
...
function isFlightLayer(layerId) {
  return layerId === 'flights' || layerId === 'military' || aircraftSources.has(layerId);
}
...
function currentTrackedFlightSubject() {
  ...
  const subjects = aircraftSourceIds().map((id) => aircraftSource(id).getTrackedSubject?.());
...
function alternativeFlightAvailable({
  ...
  return aircraftSourceIds().filter((id) => !targetLayer || targetLayer === id).some((layerId) => (
    aircraftSource(layerId).getNearby(state.subject.position, AWARENESS_MAX_FLIGHT_SEARCH_RADIUS_M,
      2, { includeHidden: true }).some((item) => aircraftClassMatchesFilter(item, aircraftClass)
...
function closestFlightWithinRadius(radiusM, visitedKeys, excludeVisited, {
  ...
  const candidates = aircraftSourceIds().filter((id) => !targetLayer || targetLayer === id)
    .flatMap((layerId) => aircraftSource(layerId).getNearby(state.subject.position, radiusM,
      layerId === 'flights' ? 25000 : 5000, { includeHidden: true })
      .filter((item) => aircraftClassMatchesFilter(item, aircraftClass))
      .map((item) => ({ layerId, id: String(item.icao24 || item.id), item }))).filter((target) => {
...
function historySourceItem(subject) {
  ...
  const layer = aircraftSource(subject.layerId);
...
function simulationSubjectHtml(subject) {
  if (!subject.simulation) return '';
  const number = (value, unit) => Number.isFinite(value) ? `${Math.round(value).toLocaleString('en-US')} ${unit}` : `— ${unit}`;
  const parts = [subject.sourceDescription || subject.source || 'SENTRY simulation', subject.aircraftType || 'Aircraft',
    number(subject.altitudeFt, 'ft'), number(subject.speedKt, 'kt'), number(subject.headingDeg, '°'),
    subject.emergencyStatus === 'DECLARED' ? 'EMERGENCY DECLARED' : 'No declared emergency'];
  if (subject.stale) parts.push('STALE · last snapshot');
  return `<p class="military-awareness-note" data-awareness-simulation="true">${parts.map(escapeHtml).join(' · ')}</p>`;
}
...
function renderResults() {
  ...
  const windowLabel = cohorts.some(cohort => cohort.id === 'ais-live-vessels')
    ? 'FLIGHT / VESSEL WINDOW' : 'AIRCRAFT WINDOW';
  const coverageNotice = cohorts.some(cohort => DEFERRED_DEPENDENCIES.includes(cohort.id))
    ? 'Open-source mapped/observed context. Missing broadcasts, unloaded map areas, or unmapped sites are not evidence of absence.'
    : 'Observed aircraft broadcasts may be incomplete; a missing broadcast is not evidence of absence.';
  const markup = `<div class="military-awareness-subject">${escapeHtml(subject.label)} · ${formatAwarenessDistance(AWARENESS_RADIUS_M)} ${windowLabel}</div>
    ${simulationSubjectHtml(subject)}
    ${navigationControlsHtml()}
    ${cohorts.map(rowHtml).join('')}
    <p class="military-awareness-note">${subject.simulation ? 'Simulation snapshot and live observations are separate sources and clocks; proximity is not a SENTRY conflict evaluation. ' : ''}${coverageNotice}</p>`;
  panel.hidden = false;
  if (state.panelMarkup !== markup) {
...
function resolveSubjectPosition(subject, { allowCollectionMaterialization = true } = {}) {
  ...
  const extra = aircraftSources.get(subject.layerId)?.layer;
  if (extra) {
    const current = extra.getSubject(subject.id);
    return { position: Cesium.Cartesian3.clone(current?.position || subject.position),
      presence: current && extra.hasContact(subject.id) !== false ? SUBJECT_PRESENCE.LIVE : SUBJECT_PRESENCE.MISSING };
  }
...
function resolveSubjectLabel(subject) {
  ...
  const layer = aircraftSource(subject.layerId);
  const tracked = layer.getSubject?.(subject.id) || layer.getTrackedSubject?.();
...
function refreshSelectedSubject(force = false) {
  ...
  const previousDetails = simulationSubjectHtml(state.subject);
  const previousLabel = state.subject.label;
  const extra = aircraftSources.get(state.subject.layerId)?.layer;
  if (extra) {
    const fresh = extra.getSubject(state.subject.id);
    // A removed/reset synthetic track is not a last-known live broadcast.
    if (!fresh || extra.hasContact(state.subject.id) === false) {
      clearAwarenessSubject(); return;
    }
    state.subject = { ...fresh, position, stale: fresh.stale === true };
  }
  const detailsChanged = previousDetails !== simulationSubjectHtml(state.subject);
  const nextLabel = resolveSubjectLabel(state.subject);
  const labelChanged = nextLabel !== previousLabel;
  ...
  if (!awarenessRefreshRequired({
    ...
    if (labelChanged || detailsChanged) renderResults();
...
function subjectFromContext(record) {
  if (record && aircraftSources.has(record.layerId)) {
    return aircraftSource(record.layerId).getSubject(record.subjectId || record.id);
  }
...
function attachRuntimeListeners() {
  ...
  state.preRenderRemover = state.viewer.scene.preRender.addEventListener(() => {
    ...
    if (state.subject && (isFlightLayer(state.subject.layerId) || state.subject.layerId === 'ais-live-vessels')) {
...
function releaseAircraftTracking() {
  for (const id of aircraftSourceIds()) aircraftSource(id).stopTracking?.();
}
...
async function enableDependencies(activationId = state.activationId) {
  ...
  for (const layerId of dependencyIds()) {
    if (!state.enabled) return;
...
function activateOperationalContext() {
  ...
  state.autoFocusRetryPending = false;
  // The shared selection wins over an older follow camera when a demo row was selected.
  const selectedRecord = getSelectedEntityContext({ dataManager: state.dataManager });
  const selectedExtra = selectedRecord && aircraftSources.has(selectedRecord.layerId)
    ? aircraftSource(selectedRecord.layerId).getSubject(selectedRecord.subjectId || selectedRecord.id) : null;
  const initialTrackedSubject = selectedExtra || currentTrackedFlightSubject()
    || [...aircraftSources.values()].filter(({ layer }) => sourceRegistered(layer.id))
      .map(({ layer }) => layer.getSelectedSubject?.()).find(Boolean);
  if (initialTrackedSubject) selectSubject(initialTrackedSubject);
  return enableDependencies(activationId).then(() => {
    if (!state.enabled || state.passive || activationId !== state.activationId) return;
    const trackedSubject = state.subject?.simulation ? null : currentTrackedFlightSubject();
    if (trackedSubject) {
...
function focusAttentionTarget() {
  ...
  const nearestFlight = closestToCurrentView([
    ...(sourceRegistered('military') ? militaryFlightsLayer.getAllPositions(800) : [])
      .map((item) => ({ ...item, layerId: 'military' })),
    ...(sourceRegistered('flights') ? flightsLayer.getAllPositions(1000) : [])
  ...
  if (!sourceRegistered('ais-live-vessels')) return false;
...
const militaryAwarenessLayer = {
  ...
  /** Register a simulation source; callbacks read its snapshot and own all selection/camera work. */
  registerAircraftSource(layer, { label = layer?.name, source = layer?.source } = {}) {
    if (!layer?.id || DEPENDENCIES.includes(layer.id)) throw new Error('Additional aircraft source needs a distinct id');
    for (const method of ['getTrackedSubject', 'getSelectedSubject', 'getSubject', 'getNearby',
      'getAllPositions', 'hasContact', 'trackById', 'stopTracking', 'getStats']) {
      if (typeof layer[method] !== 'function') throw new Error(`Aircraft source is missing ${method}`);
    }
    aircraftSources.set(layer.id, { layer, label: label || layer.id, source: source || 'Simulation' });
  },
  setParams(params = {}) {
  ...
  releaseCameraOwnership({ preserveVesselSelection = false, origin = 'programmatic' } = {}) {
    ...
    try {
      for (const id of aircraftSourceIds()) aircraftSource(id).stopTracking?.({ origin });
      if (!preserveVesselSelection && sourceRegistered('ais-live-vessels')) aisLiveVesselsLayer.clearSelection?.();
    } finally {
