...
for (const source of ['local', 'share']) {
  test(`reduced registry restores available layers from legacy ${source} without rewriting storage`, async () => {
    const legacy = decodeLayerStateParams(new URLSearchParams(
      'v=2&l=c.f.m.s.j&lo=f.e.1_f.m.a_c.c.v_s.c.d',
    ));
    assert.ok(legacy.enabledLayerIds.includes('cctv'), 'old codec tokens remain readable');
    const originalStored = serializeStoredLayerState(legacy);
    const storage = memoryStorage(originalStored);
    const manager = new DataLayerManager({});
    for (const id of ['sentry-demo', 'flights', 'military']) manager.register(fakeLayer(id));
    manager.finalizeRegistrations(LAYER_STATE_REGISTRY.filter(({ id }) => manager.layers.has(id)));
    const restoredIds = [];
    const restore = manager.restoreLayerState.bind(manager);
    manager.restoreLayerState = (id, ...args) => {
      restoredIds.push(id);
      return restore(id, ...args);
    };
    const share = shareSink();
    const coordinator = new LayerStateCoordinator(manager, share, { storage });
    try {
      const results = await coordinator.start(source === 'share' ? { shareLayerState: legacy } : {});
      assert.equal(coordinator.source, source);
      assert.deepEqual(restoredIds.sort(), ['flights', 'military', 'sentry-demo']);
      assert.equal(results.length, 3);
      assert.ok(results.every((result) => result.succeeded && !result.persistenceWrite));
      for (const id of ['flights', 'military', 'sentry-demo']) assert.equal(manager.isEnabled(id), true);
      for (const id of ['flights', 'military']) {
        assert.equal(manager.getLayerParams(id).models3d, true);
        assert.equal(manager.getLayerParams(id).models3dMode, 'all');
      }
      const regenerated = new URLSearchParams(encode(share.provider()));
      assert.equal(regenerated.get('l'), 'f.m.j');
      assert.equal(regenerated.get('lo'), 'f.e.1_f.m.a');
      assert.equal(storage.getItem(LAYER_STATE_STORAGE_KEY), originalStored);
      assert.deepEqual(storage.writes, []);
      assert.deepEqual(legacy.enabledLayerIds, ['cctv', 'flights', 'military', 'satellites', 'sentry-demo']);

      manager.setLayerParams('flights', { models3d: false }, { origin: 'user' });
      assert.equal(storage.writes.length, 1, 'only explicit user intent updates saved preferences');
      const saved = parseStoredLayerState(storage.getItem(LAYER_STATE_STORAGE_KEY));
      assert.deepEqual(saved.enabledLayerIds, ['flights', 'military', 'sentry-demo']);
      assert.equal(saved.options.flights.models3d, false);
      assert.equal(saved.options.flights.models3dMode, 'all');
      assert.equal(saved.options.cctv.coverageMode, 'on');
      assert.equal(saved.options.satellites.catalog, 'core');
    } finally {
      coordinator.destroy();
    }
  });
...
test('retired satellite follow is ignored while available aircraft settings still restore', async () => {
  const legacy = decodeLayerStateParams(new URLSearchParams('v=2&l=f.m.s.j&lo=s.t.25544'));
  const storage = memoryStorage(serializeStoredLayerState(legacy));
  const manager = new DataLayerManager({});
  for (const id of ['sentry-demo', 'flights', 'military']) manager.register(fakeLayer(id));
  manager.finalizeRegistrations(LAYER_STATE_REGISTRY.filter(({ id }) => manager.layers.has(id)));
  manager.resolveLayerTrackingTarget = () => assert.fail('retired tracking must not start');
  const statuses = [];
  const coordinator = new LayerStateCoordinator(manager, shareSink(), {
    storage,
    onTrackingRestoreStatus: (status) => statuses.push(status),
  ...
  try {
    await coordinator.start({ shareLayerState: legacy });
    assert.equal(coordinator.getDurableState().options.satellites.selectedSatTrackingId, null);
    assert.deepEqual(await coordinator.restoreShareTrackingSelection(), {
      status: 'skipped', reason: 'no-shared-target',
    });
    assert.equal(manager.getLayerParams('flights').models3d, false, 'historical omitted flag still means OFF');
    assert.equal(manager.getLayerParams('military').models3d, false);
