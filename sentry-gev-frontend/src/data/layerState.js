...
export const LAYER_STATE_REGISTRY = Object.freeze([
  ...
  Object.freeze({ id: 'sentry-demo', token: 'j', disposition: 'enabled-only' }),
...
export class LayerStateCoordinator {
  constructor(dataManager, shareLinkManager, {
    ...
    // The codec also reads historical layers absent from this application build.
    // Only registered production layers may take part in restore or durable intent.
    this._registry = LAYER_STATE_REGISTRY.filter(({ id }) => dataManager.layers.has(id));
    this._registeredLayerIds = new Set(this._registry.map(({ id }) => id));
    this._optionOwnerIds = new Set(this._registry.map(({ optionOwner }) => optionOwner).filter(Boolean));
  ...
  start({ shareLayerState = null, allowLocalState = true, shareCreatedAtMs = null } = {}) {
    ...
    this._durableState = this._normalizeAvailableState(selected || createDefaultLayerState());
  ...
  _normalizeAvailableState(candidate) {
    const input = candidate && typeof candidate === 'object' ? candidate : {};
    return normalizeLayerState({
      ...input,
      enabledLayerIds: Array.isArray(input.enabledLayerIds)
        ? input.enabledLayerIds.filter((id) => this._registeredLayerIds.has(String(id)))
        : [],
      options: Object.fromEntries([...this._optionOwnerIds].map((ownerId) => [
  ...
  _handleManagerChange(change) {
    if (!change || this._destroyed || !this._registeredLayerIds.has(change.layerId)) return;
  ...
  _commitExplicit(candidate) {
    this._durableState = this._normalizeAvailableState(candidate);
  ...
  async _restoreSelectedState(origin) {
    ...
    try {
      ...
      const enabled = new Set(this._durableState.enabledLayerIds);
      const settled = await Promise.allSettled(this._registry.map(async (entry) => {
      ...
      this.lastRestoreResults = settled.map((result, index) => {
        ...
        const entry = this._registry[index];
        return {
          layerId: entry.id,
  ...
  _passivelyClearTrackingSelection(selected) {
    ...
    this._durableState = this._normalizeAvailableState({
      ...this._durableState,
      options: {
