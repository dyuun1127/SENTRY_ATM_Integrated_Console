...
import { getSelectedEntityContext, selectTrackedSubjectContext } from './contextStore.js';
import { createSentryLayer } from '../sentry/layer.js';
...
function installSimulationSource(runtime) {
  const rows = new Map(['ROKAF01', 'ROKAF02'].map((id, index) => [id, {
    layerId: 'sentry-demo', id, label: id,
    position: Cesium.Cartesian3.fromDegrees(127.49 + index * 0.01, 36.7, 2000),
    simulation: true, source: 'SENTRY simulation', aircraftType: 'GENERIC JET',
    altitudeFt: 6500, speedKt: 210, headingDeg: 180, verticalRateFpm: 0,
    emergencyStatus: 'NONE', stale: false,
  }]));
  const calls = [];
  let selectedId = null, trackedId = null;
  const layer = {
    id: 'sentry-demo',
    rows, calls,
    getSubject(id) { return rows.get(id) || null; },
    getSelectedSubject() { return layer.getSubject(selectedId); },
    getTrackedSubject() { return layer.getSubject(trackedId); },
    getAllPositions(maxCount) { return [...rows.values()].slice(0, maxCount); },
    getNearby(center, range, maxCount) {
      return [...rows.values()].map(row => ({ ...row,
        distanceM: Cesium.Cartesian3.distance(center, row.position) }))
        .filter(row => row.distanceM <= range).sort((a, b) => a.distanceM - b.distanceM)
        .slice(0, maxCount);
    },
    hasContact(id) { return rows.has(id); },
    getStats() { return { count: rows.size, lastUpdate: 1, status: 'ready',
      stale: [...rows.values()].some(row => row.stale), error: null }; },
    select(id) {
      selectedId = id;
      const subject = layer.getSubject(id);
      if (!subject) return false;
      selectTrackedSubjectContext({ ...subject, id: 'sentry-demo:' + id, subjectId: id });
      runtime.dispatch('gev:awareness-subject-selected', subject);
      return true;
    },
    trackById(id) {
      if (!rows.has(id) || rows.get(id).stale) return false;
      calls.push(id); trackedId = id;
      runtime.viewer.trackedEntity = { gevTrackedId: 'sentry-demo:' + id };
      return layer.select(id);
    },
    refocusTrackedById(id) { return trackedId === id && layer.trackById(id); },
    stopTracking() { trackedId = null; runtime.viewer.trackedEntity = undefined; },
    clear(reason = 'deliberate') {
      const id = selectedId;
      selectedId = trackedId = null;
      runtime.dispatch('gev:awareness-subject-cleared', { layerId: layer.id, id, reason });
    },
  };
  runtime.layers.set(layer.id, { module: layer });
  militaryAwarenessLayer.registerAircraftSource(layer, { label: 'SENTRY Demo', source: 'SENTRY simulation' });
  return layer;
}

function removeDeferredSources(runtime, restores) {
  for (const [id, layer] of [['ais-live-vessels', aisLiveVesselsLayer],
    ['military-installations', militaryInstallationsLayer]]) {
    runtime.layers.delete(id);
    replaceMethod(layer, 'getNearby', () => { throw new Error('Removed source must not be queried'); }, restores);
    replaceMethod(layer, 'getAllPositions', () => { throw new Error('Removed source must not be queried'); }, restores);
  }
  replaceMethod(flightsLayer, 'getNearby', () => [], restores);
  replaceMethod(militaryFlightsLayer, 'getNearby', () => [], restores);
}

test('Contacts keeps a preselected simulation subject and excludes unregistered sources', async () => {
  const enabledCalls = [], restores = [];
  const runtime = installAwarenessRuntime({
    isEnabled: id => id === 'sentry-demo',
    setEnabled: async (id, enabled) => { enabledCalls.push([id, enabled]); return true; },
  });
  try {
    const source = installSimulationSource(runtime);
    removeDeferredSources(runtime, restores);
    source.select('ROKAF01');
    const priorCamera = { gevTrackedId: 'flights:old001' };
    runtime.viewer.trackedEntity = priorCamera;
    replaceMethod(flightsLayer, 'getTrackedSubject', () => ({ layerId: 'flights', id: 'old001',
      label: 'Older live follow', position: source.getSubject('ROKAF01').position }), restores);
    militaryAwarenessLayer.setParams({ passive: false });
    await nextTurn();
    const snapshot = militaryAwarenessLayer.getContextSnapshot();
    assert.equal(snapshot.subject.id, 'ROKAF01');
    assert.equal(runtime.viewer.trackedEntity, priorCamera, 'selecting Context does not claim the camera');
    assert.deepEqual(snapshot.cohorts.map(cohort => cohort.id), ['flights', 'military', 'sentry-demo']);
    assert.equal(snapshot.cohorts[2].count, 1, 'own aircraft is excluded from its simulation cohort');
    assert.equal(snapshot.cohorts[2].source, 'SENTRY simulation');
    assert.match(snapshot.cohorts[2].reason, /simulation snapshot/);
    assert.deepEqual(enabledCalls, [['flights', true], ['military', true]]);
    assert.match(runtime.panel().innerHTML, /6,500 ft.*210 kt.*180 °/);
    assert.match(runtime.panel().innerHTML, /separate sources and clocks/);
    assert.match(runtime.panel().innerHTML, /AIRCRAFT WINDOW/);
    assert.doesNotMatch(runtime.panel().innerHTML, /VESSEL WINDOW|unmapped sites/);
  } finally { restores.reverse().forEach(restore => restore()); runtime.restore(); }
});

test('Contacts preserves simulation follow and routes NEXT PREVIOUS and FOCUS to its owner', async () => {
  const runtime = installAwarenessRuntime(), restores = [];
  try {
    const source = installSimulationSource(runtime);
    removeDeferredSources(runtime, restores);
    source.trackById('ROKAF01');
    const followed = runtime.viewer.trackedEntity;
    militaryAwarenessLayer.setParams({ passive: false });
    await nextTurn();
    assert.equal(runtime.viewer.trackedEntity, followed);
    assert.equal(militaryAwarenessLayer.navigateNext(), true);
    assert.equal(militaryAwarenessLayer.getContextSnapshot().subject.id, 'ROKAF02');
    assert.equal(militaryAwarenessLayer.navigatePrevious(), true);
    assert.equal(militaryAwarenessLayer.getContextSnapshot().subject.id, 'ROKAF01');
    assert.equal(militaryAwarenessLayer.focusCurrent(), true);
    assert.deepEqual(source.calls, ['ROKAF01', 'ROKAF02', 'ROKAF01', 'ROKAF01']);
    assert.equal(runtime.viewer.trackedEntity.gevTrackedId, 'sentry-demo:ROKAF01');
  } finally { restores.reverse().forEach(restore => restore()); runtime.restore(); }
});

test('Contacts refreshes simulation details and stale state without reselecting the aircraft', async () => {
  const runtime = installAwarenessRuntime(), restores = [];
  try {
    const source = installSimulationSource(runtime);
    removeDeferredSources(runtime, restores);
    ...
    source.select('ROKAF01');
    await nextTurn();
    const position = Cesium.Cartesian3.fromDegrees(127.4901, 36.7, 2000);
    source.rows.set('ROKAF01', { ...source.getSubject('ROKAF01'), position,
      label: 'ROKAF01 updated', altitudeFt: 6250, speedKt: 190, headingDeg: 210,
      emergencyStatus: 'DECLARED', stale: true, staleReason: 'network-error' });
    await militaryAwarenessLayer.update();
    const snapshot = militaryAwarenessLayer.getContextSnapshot();
    assert.equal(snapshot.subject.altitudeFt, 6250);
    assert.equal(snapshot.subject.label, 'ROKAF01 updated');
    assert.deepEqual(snapshot.subject.position, position);
    assert.equal(snapshot.cohorts.find(cohort => cohort.id === 'sentry-demo').count, null);
    assert.match(runtime.panel().innerHTML, /6,250 ft.*190 kt.*210 °.*EMERGENCY DECLARED.*STALE/);
    source.rows.set('ROKAF01', { ...source.getSubject('ROKAF01'), emergencyStatus: 'NONE', stale: false });
    await militaryAwarenessLayer.update();
    assert.doesNotMatch(runtime.panel().innerHTML, /STALE|EMERGENCY DECLARED/);
  } finally { restores.reverse().forEach(restore => restore()); runtime.restore(); }
});

test('Contacts drops vanished simulation rows and RESET history even when the same callsign returns', async () => {
  const runtime = installAwarenessRuntime(), restores = [];
  try {
    const source = installSimulationSource(runtime);
    removeDeferredSources(runtime, restores);
    source.select('ROKAF01');
    const original = source.getSubject('ROKAF01');
    militaryAwarenessLayer.setParams({ passive: false });
    await nextTurn();
    source.rows.delete('ROKAF01');
    await militaryAwarenessLayer.update();
    assert.equal(militaryAwarenessLayer.getContextSnapshot(), null);
    assert.deepEqual(_getAwarenessNavigationStateForTest().historyKeys, []);
    source.rows.set('ROKAF01', original);
    source.select('ROKAF01');
    source.trackById('ROKAF02');
    source.clear('reset');
    assert.equal(militaryAwarenessLayer.getContextSnapshot(), null);
    assert.deepEqual(_getAwarenessNavigationStateForTest().historyKeys, []);
    source.select('ROKAF01');
    assert.deepEqual(_getAwarenessNavigationStateForTest().historyKeys, ['sentry-demo:ROKAF01']);
  } finally { restores.reverse().forEach(restore => restore()); runtime.restore(); }
});


test('live selection after a simulation subject replaces its identity and emergency details', async () => {
  const runtime = installAwarenessRuntime(), restores = [];
  try {
    const source = installSimulationSource(runtime);
    removeDeferredSources(runtime, restores);
    source.rows.set('ROKAF01', { ...source.getSubject('ROKAF01'), emergencyStatus: 'DECLARED' });
    source.select('ROKAF01');
    militaryAwarenessLayer.setParams({ passive: false });
    await nextTurn();
    assert.match(runtime.panel().innerHTML, /EMERGENCY DECLARED/);
    const live = { layerId: 'flights', id: 'abc123', label: 'CIVIL 123',
      position: source.getSubject('ROKAF01').position };
    replaceMethod(flightsLayer, 'getAllPositions', () => [{ ...live }], restores);
    replaceMethod(flightsLayer, 'hasContact', () => true, restores);
    runtime.dispatch('gev:awareness-subject-selected', live);
    const snapshot = militaryAwarenessLayer.getContextSnapshot();
    assert.equal(snapshot.subject.layerId, 'flights');
    assert.equal(snapshot.subject.id, 'abc123');
    assert.equal(snapshot.subject.simulation, undefined);
    assert.doesNotMatch(runtime.panel().innerHTML, /data-awareness-simulation|EMERGENCY DECLARED/);
    assert.deepEqual(snapshot.cohorts.map(cohort => cohort.id), ['flights', 'military', 'sentry-demo']);
    assert.equal(snapshot.cohorts[2].simulation, true);
    assert.equal(snapshot.cohorts[2].count, 2, 'simulation cohort remains separate after live selection');
  } finally { restores.reverse().forEach(restore => restore()); runtime.restore(); }
});


test('actual SENTRY layer and Contacts share namespaced selection, focus, snapshots and RESET', async () => {
  const runtime = installAwarenessRuntime(), restores = [];
  const originalNow = Date.now;
  let wallClock = originalNow();
  const session = {
    session_id: 'TEST-RUN-01', scenario_id: 'TEST-SCENARIO', elapsed_seconds: 0,
    simulation_time_utc: '2026-09-01T00:00:00Z', traffic_count: 2,
    traffic: ['SIM-01', 'SIM-02'].map((id, index) => ({
      aircraft_id: id, aircraft_type: 'GENERIC', category: 'AIRLINER', source: 'SYNTHETIC',
      x_nm: 1 + index, y_nm: 2, altitude_ft: 9000, heading_deg: 90,
      ground_speed_kt: 240, emergency_status: 'NONE',
    })),
  };
  let failReads = false;
  const source = createSentryLayer({ client: {
    async get(path) {
      if (failReads) throw new Error('offline');
      if (path === '/golden-demo/session') return structuredClone(session);
      if (path === '/reference/scenario') return { scenario_id: session.scenario_id, steps: [] };
      return {};
    },
    async command() { assert.fail('Selection, focus and snapshot refresh must never issue commands'); },
  } });
  try {
    Date.now = () => wallClock;
    removeDeferredSources(runtime, restores);
    await militaryAwarenessLayer.disable();
    runtime.viewer.dataSources = new Cesium.DataSourceCollection();
    for (const name of ['selectedEntity', 'trackedEntity']) {
      let value;
      runtime.viewer[name + 'Changed'] = new Cesium.Event();
      Object.defineProperty(runtime.viewer, name, {
        get: () => value,
        set(next) { value = next; runtime.viewer[name + 'Changed'].raiseEvent(next); },
      });
    }
    runtime.layers.set(source.id, { module: source });
    militaryAwarenessLayer.registerAircraftSource(source,
      { label: 'SENTRY Demo', source: 'SENTRY simulation' });
    await source.init(runtime.viewer);
    source.enable(); await source.update(runtime.viewer);
    assert.equal(source.select('SIM-01'), true);
    assert.equal(militaryAwarenessLayer.getContextSnapshot(), null, 'Contacts was not listening to that selection');
    assert.equal(getSelectedEntityContext().id, 'sentry-demo:SIM-01');
    assert.equal(getSelectedEntityContext().subjectId, 'SIM-01');
    militaryAwarenessLayer.setParams({ passive: false });
    await militaryAwarenessLayer.enable();
    assert.equal(militaryAwarenessLayer.getContextSnapshot().subject.id, 'SIM-01');
    assert.equal(militaryAwarenessLayer.getContextSnapshot().cohorts[2].count, 1);
    assert.match(runtime.panel().innerHTML, /SYNTHETIC · SENTRY simulation/);

    // The actual integration releases existing Context camera ownership here.
    source.setCameraPreparation(() => militaryAwarenessLayer.releaseCameraOwnership());
    assert.equal(militaryAwarenessLayer.focusCurrent(), true);
    assert.equal(runtime.viewer.trackedEntity.gevTrackedId, 'sentry-demo:SIM-01');
    assert.equal(militaryAwarenessLayer.navigateNext({ targetLayer: 'sentry-demo' }), true);
    assert.equal(runtime.viewer.trackedEntity.gevTrackedId, 'sentry-demo:SIM-02');
    assert.equal(militaryAwarenessLayer.getContextSnapshot().subject.id, 'SIM-02');
    assert.equal(militaryAwarenessLayer.navigatePrevious(), true);
    assert.equal(source.getSelectedSubject().id, 'SIM-01');

    session.elapsed_seconds = 1; session.simulation_time_utc = '2026-09-01T00:00:01Z';
    session.traffic[0].altitude_ft = 8800; session.traffic[0].emergency_status = 'DECLARED';
    await source.update(runtime.viewer); wallClock += 800; runtime.tick();
    assert.equal(militaryAwarenessLayer.getContextSnapshot().subject.altitudeFt, 8800);
    assert.match(runtime.panel().innerHTML, /8,800 ft.*EMERGENCY DECLARED/);
    failReads = true;
    await source.update(runtime.viewer); wallClock += 800; runtime.tick();
    assert.match(runtime.panel().innerHTML, /STALE/);
    failReads = false;
    session.session_id = 'TEST-RUN-02'; session.elapsed_seconds = 0;
    session.simulation_time_utc = '2026-09-01T00:00:00Z';
    await source.update(runtime.viewer); wallClock += 800; runtime.tick();
    assert.equal(militaryAwarenessLayer.getContextSnapshot(), null);
    assert.equal(getSelectedEntityContext(), null);
    assert.equal(runtime.viewer.trackedEntity, undefined);
    assert.deepEqual(_getAwarenessNavigationStateForTest().historyKeys, []);
  } finally {
    Date.now = originalNow;
    source.destroy(); restores.reverse().forEach(restore => restore()); runtime.restore();
  }
});
