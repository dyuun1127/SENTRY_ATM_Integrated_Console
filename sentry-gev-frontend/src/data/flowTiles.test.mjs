...
import { createSyntheticFlowTile } from './fixtures/synthetic-flow-tile.mjs';
import {
  decodeFlowTile,
  fetchFlowForBounds,
  tilesForBounds,
...
// Entirely synthetic grid geometry, projected into z12/x935/y1686 for tests.
const FIXTURE_TILE = { z: 12, x: 935, y: 1686 };

function loadFixture() {
  return createSyntheticFlowTile();
}

// ── decodeFlowTile against the synthetic fixture ─────────────────

test('synthetic fixture: deterministic bytes and known decoded attributes', () => {
  assert.deepEqual(loadFixture(), loadFixture());
  const segments = decodeFlowTile(loadFixture(), FIXTURE_TILE.z, FIXTURE_TILE.x, FIXTURE_TILE.y);
  assert.equal(segments.length, 64);
  assert.deepEqual(segments.slice(0, 5).map((segment) => segment.trafficLevel), [0, 0.25, 0.5, 0.75, 1]);
  assert.equal(segments[0].roadType, 'primary');
  assert.equal(segments[1].roadType, 'secondary');
  assert.equal(segments[0].closure, false);
  assert.equal(segments[63].closure, true);
  assert.equal(segments[63].trafficLevel, 0, 'closure without a speed ratio falls back to zero');
  assert.equal(segments[0].coords.length, 3);
  assert.ok(segments[0].coords[1][0] > segments[0].coords[0][0], 'first x delta is positive');
  assert.ok(segments[0].coords[2][0] < segments[0].coords[1][0], 'second x delta is negative');
});
...
test('fixture decode: segment shape is {coords, trafficLevel, roadType, closure}', () => {
  ...
  // The fixture includes an invented closure — closure decoding is exercised, not vacuous.
