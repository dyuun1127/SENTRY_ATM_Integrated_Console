...
class CockpitViewController {
  ...
  unsupportedSourceReason() {
    ...
    const selected = trackedEntity ? null : getSelectedEntityContext();
    return cockpitUnsupportedReason(trackedEntity?.gevTrackedId, selected?.layerId);
  }
  ...
  readAircraftInfo() {
    ...
    const trackedEntity = this.viewer?.trackedEntity || this.trackedEntity;
    if (this.unsupportedSourceReason()) return null;
  ...
  syncEntry() {
    ...
    const blockedReason = this.unsupportedSourceReason();
    const info = this.readAircraftInfo();
    ...
    if (this._entryAvailable === available && this._entryBlockedReason === blockedReason) return;
    this._entryAvailable = available;
    this._entryBlockedReason = blockedReason;
    if (this.entry) {
      this.entry.hidden = !available && !blockedReason;
      this.entry.disabled = Boolean(blockedReason);
      this.entry.title = blockedReason || 'Enter cockpit view';
      this.entry.setAttribute('aria-label', blockedReason || 'Enter cockpit view');
      const label = this.entry.querySelector('span:last-child');
      if (label) label.textContent = blockedReason ? 'COCKPIT · MAP ONLY' : 'COCKPIT';
    }
...
export class StyleManager {
  ...
  getCockpitState() {
    ...
    const unsupportedReason = this.cockpitView?.unsupportedSourceReason?.() || null;
    ...
    return {
      ...
      entryBlockedReason: entryAllowed || active
        ? null
        : unsupportedReason ? 'sentry-demo-unsupported' : (!gateOpen
          ? (this._contextModeChanging ? 'contacts-starting' : 'contacts-inactive')
          : 'no-tracked-aircraft'),
      entryBlockedMessage: unsupportedReason,
  ...
  controlCockpit(action, {
    ...
    if (normalized === 'enter') {
      const unsupportedReason = cockpitUnsupportedReason(
        selectedTarget ? `${selectedTarget.layerId}:${selectedTarget.id}` : '', targetLayer,
      ) || (!targetLayer && !selectedTarget ? this.cockpitView.unsupportedSourceReason?.() : null);
      if (unsupportedReason) {
        return { ok: false, action: 'control_cockpit', error: unsupportedReason, state: this.getCockpitState() };
      }
