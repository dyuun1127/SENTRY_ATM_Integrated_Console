...
import { initSentryIntegration } from './sentry/integration.js';
import { ARP } from './sentry/adapter.js';
...
async function init() {
  ...
  try {
    ...
    if (!styleManager.hasShareState) {
      if (new URLSearchParams(location.search).get('sentry') === '0') {
        loaderStatus.textContent = 'Flying to Austin, TX...';
        flyToAustin(viewer);
      } else {
        loaderStatus.textContent = 'Loading SENTRY · Cheongju airspace...';
        viewer.camera.setView({ destination: Cesium.Cartesian3.fromDegrees(ARP.lon, ARP.lat - 0.5, 145000),
    ...
    dataManager.register(sentryLayer);
    dataManager.register(flightsLayer);
    dataManager.register(militaryFlightsLayer);
    militaryAwarenessLayer.registerAircraftSource(sentryLayer, {
      label: 'SENTRY Demo', source: 'SENTRY simulation',
    });
    // CONTACTS coordinates the aircraft sources internally; it is not a fourth toggle.
    dataManager.register(militaryAwarenessLayer);
    militaryAwarenessLayer.attachDataManager(dataManager);
    // Retain historical share tokens while exposing only this demo's aircraft layers.
    const availableLayerRegistry = LAYER_STATE_REGISTRY.filter(({ id }) => dataManager.layers.has(id));
    dataManager.finalizeRegistrations(availableLayerRegistry);
    ...
    const sentryIntegration = initSentryIntegration({ dataManager, styleManager, layer: sentryLayer });
    ...
    void Promise.all([
      ...
      const revealFirstRun = () => {
        ...
        if (new URLSearchParams(location.search).get('sentry') !== '1') {
