...
test('CONTACTS preserves SENTRY as an optional aircraft companion', () => {
  assert.equal(contextAllowedLayerIds('flights').has('sentry-demo'), true);
  assert.equal(contextAllowedLayerIds('space-missions').has('sentry-demo'), false);
  assert.equal(shouldExitContextForLayerChange({
    contextMode: 'flights', globalContextEnabled: true,
    change: { type: 'visibility', origin: 'user', layerId: 'sentry-demo', enabled: false },
  }), false, 'turning off the optional demo must not tear down live Contacts');
});

test('CONTACTS restoration respects SENTRY toggles made during the session', () => {
  const snapshot = { enabledLayerIds: new Set(['sentry-demo']), userAdded: new Set(), userRemoved: new Set() };
  const change = { type: 'visibility', origin: 'user', layerId: 'sentry-demo', enabled: false };
  assert.equal(recordContextSessionUserChange({ snapshot, change, effectiveContextMode: 'flights' }), true);
  assert.equal(contextRestoreLayerIds(snapshot).has('sentry-demo'), false);
  change.enabled = true;
  assert.equal(recordContextSessionUserChange({ snapshot, change, effectiveContextMode: 'flights' }), true);
  assert.equal(contextRestoreLayerIds(snapshot).has('sentry-demo'), true);
  const initiallyOff = { enabledLayerIds: new Set(), userAdded: new Set(), userRemoved: new Set() };
  recordContextSessionUserChange({ snapshot: initiallyOff, change, effectiveContextMode: 'flights' });
  assert.equal(contextRestoreLayerIds(initiallyOff).has('sentry-demo'), true);
});
