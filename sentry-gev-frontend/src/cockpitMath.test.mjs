...
test('a SENTRY simulation owner cannot fall back to a leftover live-aircraft descriptor', () => {
  const civilian = { icao24: 'aaa077', callsign: 'OLD LIVE' };
  const military = { icao24: 'ae01ce' };
  for (const trackedId of ['sentry-demo:AAR1405', ' SENTRY-DEMO:TEST01 ']) {
    assert.equal(resolveTrackedAircraftInfo({ civilian, military, trackedId }), null);
    assert.match(cockpitUnsupportedReason(trackedId), /SENTRY.*MAP/);
  }
});

test('an untracked SENTRY selection explains the limitation without blocking later live tracking', () => {
  assert.match(cockpitUnsupportedReason('', 'sentry-demo'), /Cockpit/);
  assert.equal(cockpitUnsupportedReason('flights:aaa077', 'sentry-demo'), null);
  assert.equal(cockpitUnsupportedReason('', 'flights'), null);
  assert.equal(cockpitUnsupportedReason(), null);
});
