...
test('Global Context names its mixed contact cycle without changing the stable mode id', () => {
  ...
  assert.match(match[0], /title="Inspect SENTRY simulation and nearby live aircraft\. Sources and clocks remain separate\."/);
...
test('Global Context standby describes both chooser modes', () => {
  ...
  assert.match(match[0], /CONTACTS — SENTRY · live aircraft/);
