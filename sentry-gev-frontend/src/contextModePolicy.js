...
const CONTEXT_MODE_COMPANIONS = Object.freeze({ flights: new Set(['sentry-demo']) });
function isContextCompanion(contextMode, layerId) {
  return CONTEXT_COMPANIONS.has(layerId) || Boolean(CONTEXT_MODE_COMPANIONS[contextMode]?.has(layerId));
}
...
export function recordContextSessionUserChange({ snapshot, change, effectiveContextMode = null }) {
  ...
  const isCompanion = isContextCompanion(effectiveContextMode, change.layerId);
  if (change.enabled) {
