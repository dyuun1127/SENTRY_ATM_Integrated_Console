...
/** Explain an explicitly unsupported simulation source without changing legacy IDs. */
export function cockpitUnsupportedReason(trackedId = '', selectedLayerId = '') {
  const key = String(trackedId || '').trim().toLowerCase();
  if (key.startsWith('sentry-demo:') || (!key && selectedLayerId === 'sentry-demo')) {
    return 'SENTRY 시연 항공기는 MAP 추적을 사용합니다. Cockpit은 공개 실시간 항공기에만 지원됩니다.';
  }
  return null;
}
...
export function resolveTrackedAircraftInfo({
  ...
  // A demo entity must not inherit a former live track's metadata during handoff.
  // Other legacy IDs and unstamped trackers retain their original precedence.
  if (cockpitUnsupportedReason(trackedId)) return null;
  const candidates = [];
