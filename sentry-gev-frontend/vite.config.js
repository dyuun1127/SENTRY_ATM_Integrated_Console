...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        '/sentry-api': {
          target: env.SENTRY_BACKEND_URL || 'http://127.0.0.1:8782',
          changeOrigin: false,
          rewrite: (path) => path.replace(/^\/sentry-api(?=\/|$)/, '/api/v1'),
        },
      },
    ...
    define: {
      'import.meta.env.SENTRY_BACKEND_URL': JSON.stringify(env.SENTRY_BACKEND_URL || 'http://127.0.0.1:8782'),
